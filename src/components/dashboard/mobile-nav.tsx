    ...
    <>
      <header className="glass-surface-solid sticky top-0 z-30 flex h-14 items-center justify-between border-b border-ink/[0.06] px-4 md:hidden">
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full text-ink/50 transition-colors active:bg-ink/[0.08]"
        >
          <MenuIcon className="h-5 w-5" />
        </button>
      </header>

      {/* Sempre montati (non condizionati da `aperto`) così l'apertura E la
          chiusura animano entrambe via transition, non solo l'ingresso. */}
      <div
        className={clsx(
          "fixed inset-0 z-40 bg-ink/30 backdrop-blur-sm transition-opacity duration-300 ease-glass md:hidden",
      ...
      <div
        className={clsx(
          "glass-surface-solid fixed inset-y-0 left-0 z-40 flex w-72 max-w-[85vw] flex-col shadow-2xl",
          "transition-transform duration-300 ease-glass md:hidden",
      ...
      >
        <div className="flex items-center justify-between gap-2.5 px-5 py-5">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full text-ink/40 hover:bg-ink/[0.06]"
          >
        ...
        <div className="mt-auto border-t border-ink/[0.06] p-4">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-[13px] font-medium text-ink/80">{nome}</p>
